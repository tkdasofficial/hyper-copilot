...
export async function createUniversalWebSocket(url: string): Promise<IWebSocket> {
  ...
  if (
    ...
    typeof (globalThis as unknown as { WebSocket?: new (u: string) => IWebSocket }).WebSocket ===
      "function"
  ) {
    const WSClass = (globalThis as unknown as { WebSocket: new (u: string) => IWebSocket })
      .WebSocket;
  ...
  if (typeof process !== "undefined" && process.versions && process.versions.node) {
    try {
      ...
      // @ts-expect-error - optional dependency in Node environments
      const wsModule: unknown = await import("ws");
      const resolved =
        (wsModule as { default?: new (u: string) => IWebSocket })?.default || wsModule;
      const WsConstructor = resolved as unknown as new (u: string) => IWebSocket;
