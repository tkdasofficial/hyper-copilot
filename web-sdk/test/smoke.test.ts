...
async function runTests() {
  ...
  assert(
    HyperVisionEngine.getEngineBrand() === "Hyper Vision Agent Web SDK",
    "Engine branding matches",
  );
  ...
  assert(
    spline.length === 21,
    `Human C-Spline generates correct step count (got ${spline.length})`,
  );
  ...
  assert(
    ...
    "Stealth script injects navigator.webdriver spoof",
  );
  ...
  assert(
    ddgUrl.includes("html.duckduckgo.com/html/?q=TypeScript%20CDP%20Engine"),
    "DDG URL builder encodes query",
  );

  const unpacked = DuckDuckGoSearch.unpackDdgRedirect(
    "//duckduckgo.com/l/?uddg=https%3A%2F%2Fgithub.com%2Ftest&rut=1",
  );
  assert(
    unpacked === "https://github.com/test",
    `DDG Redirect unpacker resolves target (${unpacked})`,
  );
  ...
  try {
    ...
    assert(
      evalRes.success && evalRes.value === 4,
      `Runtime.evaluate: '2 + 2' === ${evalRes.value}`,
    );
    ...
    `);
    assert(
      objRes.success && objRes.value?.sdk === "Hyper Vision Web SDK",
      "Object script evaluation succeeded",
    );
    ...
    assert(
      ...
      `Button state updated via synthetic click ('${btnText.value}')`,
    );
    ...
    assert(
      screenshot.success && screenshot.base64Data.length > 100,
      `Viewport screenshot captured (${screenshot.base64Data.length} chars base64)`,
    );
