      ...
      const res = await fetch(GITHUB_DISPATCH_URL, {
        ...
        body: JSON.stringify({
          ...
          client_payload: {
            video_id: video.id,
            prompt: video.prompt,
            mode: video.aspect_ratio === "16:9" || Number(video.duration_seconds) > 60 ? "long" : "short",
            identity: {
              video_id: video.id,
            ...
            visual: {
              visual_style: video.image_style,
            ...
            audio: {
              ...
              bgm: video.motion_template !== "bgm_off" ? "true" : "false",
            },
            timing: {
              duration_seconds: String(video.duration_seconds),
              duration_minutes: String(Math.max(1, Math.round(Number(video.duration_seconds || 15) / 60))),
            },
            caption: {
              captions: video.captions ? "true" : "false",
              caption_style: video.caption_style ?? "Dynamic",
              caption_size: captionSizeToken(video.caption_style),
        ...
        const ghRes = await fetch(GITHUB_DISPATCH_URL, {
          ...
          body: JSON.stringify({
            ...
            client_payload: {
              ...
              mode: videoConfig.aspect_ratio === "16:9" || Number(videoConfig.duration_seconds) > 60 ? "long" : "short",
              identity: {
                video_id: newVideoId,
                user_id: userId,
              },
              ...
              audio: {
                ...
                bgm: videoConfig.motion_template !== "bgm_off" ? "true" : "false",
              },
              timing: {
                duration_seconds: String(videoConfig.duration_seconds),
                duration_minutes: String(Math.max(1, Math.round(Number(videoConfig.duration_seconds || 15) / 60))),
              },
              caption: {
                captions: videoConfig.captions ? "true" : "false",
                caption_style: videoConfig.caption_style,
                caption_size: captionSizeToken(videoConfig.caption_style),
