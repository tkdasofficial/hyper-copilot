...
3. Scene images from prompt + `image_style` (+ `negative_prompt`) come from Pixazo AI
   (`https://api.pixazo.ai`, images only), falling back to Cloudflare Workers AI image
   models (`@cf/black-forest-labs/flux-1-schnell`, then SDXL variants) if Pixazo fails.
4. Everything else is Cloudflare Workers AI: script/scene-prompt generation with the Llama
   instruct models, and narration TTS for `voice_gender` (`@cf/deepgram/aura-1`, MeloTTS
   fallback). Pixazo exposes no TTS API, so it is never called for audio.
5. FFmpeg → composes images + audio + captions at the requested aspect ratio into an MP4.
   Captions are written as a real ASS file with `PlayResX/PlayResY` set to the canvas (44px at
   1080x1920, bottom-centre, 3-4 words per cue) — `force_style` on an SRT sizes against libass'
   384x288 default and rendered full-screen text. Clip length follows the narration
   (`narration + 0.6s`, capped at the selected duration) and the audio track is `apad`-ed to that
   length. Narration is retimed to fill the selected runtime, preventing short output or a long
   silent gap when a voice provider speaks faster than expected.
