...
export class ProcessLauncher {
  ...
  constructor() {
    this.isNodeRuntime = typeof process !== "undefined" && Boolean(process.versions?.node);
  }
  ...
  private async waitForWsDebuggerUrl(port: number, timeoutMs: number): Promise<string> {
    ...
    throw new TimeoutError(
      `Failed to obtain WebSocket Debugger URL from Chromium on port ${port} within ${timeoutMs}ms`,
    );
  }
  ...
  public kill(): void {
    if (
      this.childProcess &&
      typeof (this.childProcess as { kill?: (signal: string) => void }).kill === "function"
    ) {
      try {
