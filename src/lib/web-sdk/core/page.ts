...
export class Page {
  ...
  public async navigate(url: string, timeoutMs = 30000): Promise<NavigationResult> {
    ...
    const timeoutPromise = new Promise<void>((_, reject) => {
      setTimeout(
        () => reject(new TimeoutError(`Navigation to '${url}' timed out after ${timeoutMs}ms`)),
        timeoutMs,
      );
    });
  ...
  public async evaluateScript<T = unknown>(script: string): Promise<EvalResult<T>> {
    ...
    if (!res.success || !res.result?.result) {
      return {
        ...
        type: "undefined",
        errorDescription:
          res.errorMessage || res.result?.exceptionDetails?.text || "Evaluation failed",
      };
    }
  ...
  public async querySelector(selector: string): Promise<number> {
    ...
    const queryRes = await this.dispatcher.domQuerySelector(
      docRes.result.root.nodeId,
      selector,
      this.sessionId,
    );
  ...
  public async querySelectorAll(selector: string): Promise<number[]> {
    ...
    const queryRes = await this.dispatcher.domQuerySelectorAll(
      docRes.result.root.nodeId,
      selector,
      this.sessionId,
    );
  ...
  public async getElementCenter(
    selectorOrNodeId: string | number,
  ): Promise<ElementCoordinates | null> {
  ...
  public async clickAt(x: number, y: number): Promise<boolean> {
    await this.dispatcher.inputDispatchMouseEvent(
      "mouseMoved",
      x,
      y,
      "none",
  ...
  public async type(selector: string, text: string, delayMs = 10): Promise<boolean> {
    ...
    for (const char of text) {
      await this.dispatcher.inputDispatchKeyEvent(
        "keyDown",
        char,
        char,
        char,
  ...
  public async captureScreenshotBase64(format = "png", quality = 80): Promise<string> {
    const res = await this.dispatcher.pageCaptureScreenshot(
      format,
      quality,
      undefined,
      true,
  ...
  public async setViewport(width: number, height: number, scale = 1.0): Promise<boolean> {
    const res = await this.dispatcher.emulationSetDeviceMetricsOverride(
      width,
      height,
      scale,
      false,
